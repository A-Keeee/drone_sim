...
double ArcLengthTable::closestU(const BSpline&s,const Eigen::Vector3d&p,double guess)const{if(us_.empty())return 0;double best=clamp(guess,0.0,s.maxU()),d=(s.position(best)-p).squaredNorm();
  for(double u:us_){double nd=(s.position(u)-p).squaredNorm();if(nd<d){d=nd;best=u;}}for(int i=0;i<8;++i){Eigen::Vector3d r=s.position(best)-p;Eigen::Vector3d v=s.velocity(best);Eigen::Vector3d a=s.acceleration(best);double h=v.squaredNorm()+r.dot(a);if(std::abs(h)<1e-9)break;best=clamp(best-clamp(r.dot(v)/h,-0.4,0.4),0.0,s.maxU());}return best;}
