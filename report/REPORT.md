...
- 规划测试覆盖需要升高绕行的 3D A*，以及五障碍场景中高密度 B-spline 的体素碰撞检查。

### 悬停实验 `(0,0,1.5)`

多包重构后的 10 s 本机实跑：

- 最终真值位置约 `(0.045, 0.076, 1.376) m`；
- 最终位置误差约 `0.152 m`；
- 误差小于 0.3 m 验收阈值；
- 稳态电机转速约 5100 RPM，无持续饱和。
...
修复静态地图重复触发规划后，使用 `sim.launch.py` 完成 40 s 本机实跑：

- 最终真值位置约 `(7.935, 0.152, 1.486) m`；
- 最终位置误差约 `0.166 m`；
- 实际轨迹到五个原始障碍盒体表面的最小距离约 `0.607 m`；
